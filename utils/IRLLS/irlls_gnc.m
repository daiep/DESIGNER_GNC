function [ reject, dt, conv, fa, md ] = irlls_gnc( dwi, mask, g_gnc, b_gnc, sigma, options )
...
%% Create B-matrix
    if options.Kurtosis
        ...
        % daiep
        bmat = cat(2, ones([ndwi,1, s0(1:3)]), -repmat(b,[1,6]).*g_gnc(:,D_ind(:,1), :, :, :)...
            .*g_gnc(:,D_ind(:,2), :, :, :)*diag(D_cnt), (1/6)*repmat(b,[1,15]).^2 ...
            .*g_gnc(:,W_ind(:,1), :, :, :).*g_gnc(:,W_ind(:,2), :, :, :).*g_gnc(:,W_ind(:,3), :, :, :)...
            .*g_gnc(:,W_ind(:,4), :, :, :)*diag(W_cnt));
    else
        ind = [1 1; 1 2; 1 3; 2 2; 2 3; 3 3];
        cnt = [1 2 2 1 2 1];
        % bmat = [ones([ndwi,1]), -repmat(b,[1,6]).*g(:,ind(:,1)).*g(:,ind(:,2))*diag(cnt)];
        % daiep
        bmat = cat(2, ones([ndwi,1, s0(1:3)]), -repmat(b,[1,6]).*g_gnc(:,ind(:,1), :, :, :)...
            .*g_gnc(:,ind(:,2), :, :, :)*diag(cnt));
    end
    nparam = size(bmat,2);
    ndof = ndwi - nparam;
...
%             e = log(dwi(:,i)) - bmat*dt_;
            % daiep
            dt_ = bmat(:, :, i)\log(dwi(:,i));
            w = exp(bmat(:, :, i)*dt_);
            dt_ = (bmat(:, :, i).*repmat(w,[1,nparam]))\(log(dwi(:,i)).*w);
            e = log(dwi(:,i)) - bmat(:, :, i)*dt_;
