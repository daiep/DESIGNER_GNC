function [b0, dt] = dki_fit_gnc(dwi, grad, mask, constraints, outliers, maxbval)
% DKI fitting with Gradient Nonlinearity Correction (GNC)
% (c) Erpeng Dai, Stanford University

    % **** Original dki_fit in DESIGNER notes begin ****
    ...
    % **** Original dki_fit in DESIGNER notes end ****
...
%     end
    ...
    % daiep
    bval_mean=mean(bval(:, :), 2); % mean bval over the whole volume
    list = (bval_mean<=maxbval);
    bval_max=max(bval_mean);
    
    dwi = dwi(:,:,:,list);
    % grad = grad(list, :);
    grad = grad(list, :, :, :, :); % daiep
...
%     bval = grad(:, 4);
    ...
    outliers = custom_vectorize(outliers, mask);
    nvoxels = size(dwi,2);
    % daiep
    ...
    bD = D_cnt(ones(ndwis, 1), :).*grad(:,D_ind(:, 1), :, :, :).*grad(:,D_ind(:, 2), :, :, :);
    bW = W_cnt(ones(ndwis, 1), :).*grad(:,W_ind(:, 1), :, :, :).*grad(:,W_ind(:, 2), :, :, :)...
        .*grad(:,W_ind(:, 3), :, :, :).*grad(:,W_ind(:, 4), :, :, :);
    
    ind0=find(mask>0);
    
    if size(s0, 2)<=2 % w/o GNC bval: [ndwi, 1]
        % b = [bS, -repmat(bval, [1 6]).*bD, (repmat(bval, [1 15]).^2/6).*bW];
        b = cat(2, bS, -repmat(bval, [1 6]).*bD, (repmat(bval, [1 15]).^2/6).*bW);
        
        bval=repmat(bval, [1 1 size(ind0, 1)]);
        b=repmat(b, [1 1 size(ind0, 1)]);
        % daiep: better mitigate noise bias similar to FSL dtifit
        parfor i=1:nvoxels
            ind10 = find(dwi(:, i)./b00mean(:, i) > exp(-bval(:, :, i)*5)); % assume maxD=5 um^2/ms
    ...
    if any(constraints) 
        ...
        if constraints(3)>0
            ...
            % daiep
            C = [C; [zeros(ndir, 1), 3/bval_max*D_cnt(ones(ndir, 1), :).*dir(:,D_ind(:, 1)).*dir(:,D_ind(:, 2)), -W_cnt(ones(ndir, 1), :).*dir(:,W_ind(:, 1)).*dir(:,W_ind(:, 2)).*dir(:,W_ind(:, 3)).*dir(:,W_ind(:, 4))]];
        end
        d = zeros([1, size(C, 1)]);
        ...
        parfor i = 1:nvoxels
            try
                ...
                % daiep: better mitigate noise bias similar to FSL dtifit
                ind10 = find(dwi(in_, i)./b00mean(1, i) > exp(-bval(in_, :, i)*5)); % assume maxD=5 um^2/ms
                b_ = b(in_, :, i);
                logdwii = log(dwi(in_,i));
                dt(:,i) = lsqlin(Wi(ind10, ind10)*b_(ind10, :), Wi(ind10, ind10)*logdwii(ind10, :), ...
                    -C, d, [],[],[],[],[],options);
            catch
                dt(:, i) = 0;
            end
        ...
        parfor i = 1:nvoxels
            ...
            if isempty(b_) || cond(b_)>1e15
                ...
                % daiep: better mitigate noise bias similar to FSL dtifit
                ind10 = find(dwi(in_, i)./b00mean(1, i) > exp(-bval(in_, :, i)*5)); % assume maxD=5 um^2/ms
                dt(:,i) = (Wi(ind10, ind10)*b_(ind10, :))\(Wi(ind10, ind10)*logdwii(ind10, :));
...
% function [s, mask] = vectorize(S, mask) % daiep: Rename it to not violate the built-in naming of matlab
function [s, mask] = custom_vectorize(S, mask)
    ...
    % daiep: Double precision is required for parallel computation.
    % Not sure why
    % s=single(s);
end
