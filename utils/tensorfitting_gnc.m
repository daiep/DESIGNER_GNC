function tensorfitting_gnc(root,outdir,detectoutliers,cumulants,dti,dki,wmti,fitconstraints,akc,DKIroot,fitWDKI,smooth)
...
% daiep: compatible with Gradient Non-linearity Correction (GNC)
% bval / bvec: nominal bval / bvec; bval_gnc / bvec_gnc: with GNC
    ...
    % daiep
    bvallist_gnc = dir(fullfile(root,'dwi_designer_bval_gnc.mat'));
    bveclist_gnc = dir(fullfile(root,'dwi_designer_bvec_gnc.mat'));
    if ~isempty(bvallist_gnc) && ~isempty(bveclist_gnc)
        bvaldir = bvallist_gnc.name;
        bvecdir = bveclist_gnc.name;
        load(fullfile(root,bvaldir)); bval_gnc = bval_gnc(1:ndwis, :, :, :, :); bval_gnc = bval_gnc./1000;
        load(fullfile(root,bvecdir)); bvec_gnc = bvec_gnc(1:ndwis, :, :, :, :);
    else
        bval_gnc=bval;
        bvec_gnc=bvec;
    end
    ...
    if detectoutliers
        ...
        % daiep: GNC not compatible with the "-outliers" option yet
    else
        disp(['... fitting with constraints ',num2str(constraints)])
        if dti && ~dki
            % [b0, dt] = dti_fit(dwi,[bvec,bval],mask);
            % daiep
            [b0,dt] = dti_fit_gnc(dwi,cat(2, bvec_gnc, bval_gnc),mask);
        else
            % [b0,dt] = dki_fit(dwi,[bvec,bval],mask,constraints,[],maxbval);
            % daiep
            [b0,dt] = dki_fit_gnc(dwi,cat(2, bvec_gnc, bval_gnc),mask,constraints,[],maxbval);
        end
    end
    ...
    if akc
         ...
         % daiep
         [dt,dwi,akc_out] = correctDt_gnc(dt,dwi,mask,bval_gnc,bvec_gnc);
         disp(['N outliers = ',num2str(sum(akc_out(:)))]);
         
         % daiep: avoid error in later dti/dki fitting from akc
         dwi_b0=dwi(:, :, :, 1);
         ind0=find(dwi_b0<=0 | isnan(dwi_b0) | isinf(dwi_b0));
         mask(ind0)=0;
...
%         disp(['N outliers = ',num2str(sum(akc_out(:)))]);
    ...
    if dti
        ...
        list = round(bval) == 0 | round(bval) <= 2; % daiep
        dwi_dti = dwi(:,:,:,list);
        % bvec_dti = bvec(list,:);
        % bval_dti = bval(list);
        % [b0_dti, dt_dti, md_dti, rd_dti, ad_dti, fa_dti] = dti_fit(dwi_dti,[bvec_dti,bval_dti],mask);
        % daiep
        bvec_dti_gnc = bvec_gnc(list, :, :, :, :);
        bval_dti_gnc = bval_gnc(list, :, :, :, :);
        [b0_dti, dt_dti, md_dti, rd_dti, ad_dti, fa_dti] = ...
            dti_fit_gnc(dwi_dti,cat(2, bvec_dti_gnc, bval_dti_gnc),mask);
    ...
    % daiep
    function[dt,dwi,akc_out] = correctDt_gnc(dt,dwi,mask,bval_gnc,bvec_gnc)
        akc_out = outlierdetection(dt,mask);
        akc_out(isnan(akc_out)) = 0;
        ...
        % daiep
        [~,dt_] = dki_fit(dwi,cat(2, bvec_gnc, bval_gnc),logical(akc_out),constraints,[],3);
        for d = 1:size(dt,4)
            dtf_ = dt_(:,:,:,d); 
            dto_ = dt(:,:,:,d);
